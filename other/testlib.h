 ...
 *(mapped to argv[2], can be judged by checker later), reads program output from
 *ouf (mapped to stdin), writes output to program via stdout (use cout, printf,
 *etc).
 */
  ...
  public:
	...
	std::vector<T> partition(int size, T sum, T min_part) {
		...
		if (*std::min_element(result.begin(), result.end()) < min_part)
			__testlib_fail(
				"random_t::partition: partition min is expeced to be to less "
